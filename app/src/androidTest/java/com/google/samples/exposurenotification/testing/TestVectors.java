...
/**
 * Java version of Exposure Notification test vectors (v1.2).
 *
 * <p>{@link #get_TEMPORARY_TRACING_KEY} is a random key generated at {@link
 * #KEY_GENERATION_NSECONDS}.
 *
 * <p>Derived from those values:
 *
 * <ul>
 *   <li>{@link #CTINTERVAL_NUMBER_OF_GENERATED_KEY} is derived from the generation time.
 *   <li>{@link #get_RPIK} returns the expected Rolling Proximity Indicator Key
 *   <li>{@link #get_AEMK} returns the expected Associated Metadata Encryption Key
 * </ul>
 *
 * <p>Given the associated metadata in {@link #get_BLE_METADATA}, the expected values at each 10
 * minute interval are provided in {@link #ADVERTISED_DATA}, e.g.
 *
 * <pre>{@code
 * for (int i = 0; i < 144; i++) {
 *   byte[] rpi = generateRpi(i, TestVectors.get_RPIK());
 *   byte[] aem = generateAem(i, TestVectors.get_AEMK(), testVectors,get_BLE_METADATA());
 *   assertThat(rpi).isEqualTo(ADVERTISED_DATA[i].get_RPI());
 *   assertThat(aem).isEqualTo(ADVERTISED_DATA[i].get_AEM());
 * }
 * }</pre>
 *
 * <p>All of the byte array "constants" are accessed through static functions which return a fresh
 * copy of the master array, to avoid any possibility of corruption by the tests.
 *
 * <p>The original C++ header is in TestVectors.h.txt.
 */
public class TestVectors {
    ...
    public static byte[] asBytes(int... ints) {
        ...
        for (int i = 0; i < ints.length; i++) {
            int value = ints[i];
            Assert.assertTrue(Range.closed(0, 255).contains(value));
            bytes[i] = (byte) value;
        }
        return bytes;
    }
    ...
    public static final ImmutableList<AdvertisedData> ADVERTISED_DATA =
            ImmutableList.of(
